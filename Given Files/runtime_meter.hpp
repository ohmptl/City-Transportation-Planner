...
namespace rt {
  ...
  struct RunTimer {
    ...
    std::int64_t stop_ms() const noexcept {
  ...
  struct ScopeTimer {
    ...
    ~ScopeTimer() {
      ...
      auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(clock_type::now() - t0).count();
  ...
  inline void print_ms(const char* label, std::int64_t ms) {
#if RUNTIME_TIMER_ENABLE
    std::fprintf(stderr, "%s=%lld\n", label, static_cast<long long>(ms));
    std::fflush(stderr);
#endif
